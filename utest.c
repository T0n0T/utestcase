...
static struct utest local_utest = {UTEST_PASSED, 0, 0, 0};
...
static rt_tick_t tick_count(rt_tick_t start, rt_tick_t end)
{
    rt_tick_t duration;
    if (start > end) {
        duration = RT_TICK_MAX - start + end;
    }else {
        duration = end - start;
    }
    return duration;
    ...
    for (index = 0; index < tc_loop; index ++)
    {
        ...
        while(i < tc_num)
        {
            ...
            if (tc_table[i].tc != RT_NULL)
            {
                tick = rt_tick_get();
                tc_table[i].tc();
                time = tick_count(tick, rt_tick_get());
                if (local_utest.failed_num == 0)
                {
                    LOG_I("[  PASSED  ] [ result   ] testcase (%s) using %d ms", tc_table[i].name, time);
                }
                else
                {
                    LOG_E("[  FAILED  ] [ result   ] testcase (%s) using %d ms", tc_table[i].name, time);
                }
            }
            else
            {
...
void utest_unit_run(test_unit_func func, const char *unit_func_name)
{
    ...
    if (func != RT_NULL)
    {
        local_utest.duration = rt_tick_get();
        func();
    }
}
...
{
    local_utest.duration = tick_count(local_utest.duration, rt_tick_get());
    if (!(value))
    {
        local_utest.error = UTEST_FAILED;
        local_utest.failed_num ++;

        LOG_E("[  ASSERT  ] [ unit     ] at (%s); func: (%s:%d); msg: (%s) using %d ms", file_basename(file), func, line, msg, local_utest.duration);
    }
    else
    {
        if (utest_log_lv == UTEST_LOG_ALL)
        {
            LOG_D("[    OK    ] [ unit     ] (%s:%d) is passed,using %d ms", func, line, local_utest.duration);
        }
        local_utest.error = UTEST_PASSED;
